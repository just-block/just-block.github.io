...
- **Prefer `snapshot`** to inspect pages. If a snapshot fails (e.g. `encode_error`), fall back to `screenshot --out /tmp/file.png` and read the image, combined with `get` commands (`get title`, `get url`, `get text`) for specific data.
